...
> 版本：v1.2 | 日期：2026-06-14 | 作者：产品通 | 变更：新增数据汇总、数据分析、自动生成月报功能规划
...
- [ ] **P0**：创建 `docs/PRODUCT_DESIGN.md`（✅ 本文档即产物，v1.2 已更新）
- [ ] **P0**：补全 `manifest.json`（当前缺失，PWA 不可用）
- [ ] **P0**：数据模型迁移脚本 — 补全 `meetingRef`/`trainingRef` 等外键字段
- [ ] **P0**：所有时间字段统一为 `YYYY-MM-DD` 格式（为月报聚合做准备）
- [ ] **P0**：所有台账模块补全 `category` 字段（为分组统计做准备）
- [ ] **P0**：驾驶舱指标从"计数"改为"比率"
- [ ] **P1**：代码拆分第一步 — 至少将 CSS 提取到独立文件
- [ ] **P1**：字段命名规范化（`content`/`note` 语义统一）
- [ ] **P1**：台账创建/更新时自动同步工作记录
- [ ] **P1**：会议日程模块 MVP（增删改查 + 日历视图 + 待办联动）
- [ ] **P1**：培训记录模块 MVP（增删改查 + 证书到期提醒）
