...
const STATE = { TODO:'未开始', DOING:'进行中', CLOSED:'已关闭' };

function mapStatusToState(ledgerKey, status){
  const s=String(status||'').trim();
  switch(ledgerKey){
    case 'todos':     return s==='进行中' ? STATE.DOING : (s==='已完成'||s==='已取消') ? STATE.CLOSED : STATE.TODO;
    case 'contracts': return s==='执行中' ? STATE.DOING : (s==='已完成'||s==='已终止') ? STATE.CLOSED : STATE.TODO;
    case 'purchases': return s==='申请中' ? STATE.TODO : (s==='已采购'||s==='已完成'||s==='已取消') ? STATE.CLOSED : STATE.DOING;
    case 'expenses':  return s==='待报销' ? STATE.TODO : (s==='已报销'||s==='已取消') ? STATE.CLOSED : STATE.DOING;
    case 'agencys':   return s==='待委托' ? STATE.TODO : s==='已委托' ? STATE.DOING : (s==='已完成'||s==='已流标'||s==='已取消') ? STATE.CLOSED : STATE.DOING;
    case 'handovers': return (s==='进行中'||s==='交接中') ? STATE.DOING : (s==='已完成'||s==='已取消'||s==='已结束') ? STATE.CLOSED : STATE.TODO;
    case 'funds':     return s==='进行中' ? STATE.DOING : (s==='已完成'||s==='已关闭') ? STATE.CLOSED : STATE.TODO;
...
function isOverdue(rec, ledgerKey){
  ...
  if(!due) return false;
  return rec.state!==STATE.CLOSED && due < new Date().toISOString().slice(0,10);
}
...
// 1) 原生 status → 统一 state
eq('todos 进行中', mapStatusToState('todos','进行中'), STATE.DOING);
eq('todos 已完成', mapStatusToState('todos','已完成'), STATE.CLOSED);
...
eq('expenses 已报销',  mapStatusToState('expenses','已报销'), STATE.CLOSED);
...
eq('handovers 已完成', mapStatusToState('handovers','已完成'), STATE.CLOSED);
